...
public class PushJobDetailsTest {
  ...
  private void validatePushJobMetrics(boolean isSucceeded, boolean isUserError, boolean isIncrementalPush) {
    /*    double batchJobSuccessExpectedSimpleSampledCount = 0.0;
    double incrementalJobSuccessExpectedSimpleSampledCount = 0.0;
    double batchJobFailedUserErrorExpectedSimpleSampledCount = 0.0;
    double batchJobFailedNonUserErrorExpectedSimpleSampledCount = 0.0;
    double incrementalJobFailedUserErrorExpectedSimpleSampledCount = 0.0;
    double incrementalJobFailedNonUserErrorExpectedSimpleSampledCount = 0.0;*/

    Map<String, ? extends Metric> metrics = metricsRepository.metrics();
    if (isSucceeded) {
      if (isIncrementalPush) {
        incrementalJobSuccessExpected += 1.0;
        // incrementalJobSuccessExpectedSimpleSampledCount = 1.0;
      } else {
        batchJobSuccessExpected += 1.0;
        // batchJobSuccessExpectedSimpleSampledCount = 1.0;
      }
    } else {
      if (isUserError) {
        if (isIncrementalPush) {
          incrementalJobFailedUserErrorExpected += 1.0;
          // incrementalJobFailedUserErrorExpectedSimpleSampledCount = 1.0;
        } else {
          batchJobFailedUserErrorExpected += 1.0;
          // batchJobFailedUserErrorExpectedSimpleSampledCount = 1.0;
        }
      } else {
        if (isIncrementalPush) {
          incrementalJobFailedNonUserErrorExpected += 1.0;
          // incrementalJobFailedNonUserErrorExpectedSimpleSampledCount = 1.0;
        } else {
          batchJobFailedNonUserErrorExpected += 1.0;
          // batchJobFailedNonUserErrorExpectedSimpleSampledCount = 1.0;
        }
      }
    }
    /*double finalIncrementalJobSuccessExpectedSimpleSampledCount = incrementalJobSuccessExpectedSimpleSampledCount;
    double finalBatchJobSuccessExpectedSimpleSampledCount = batchJobSuccessExpectedSimpleSampledCount;
    double finalBatchJobFailedUserErrorExpectedSimpleSampledCount = batchJobFailedUserErrorExpectedSimpleSampledCount;
    double finalBatchJobFailedNonUserErrorExpectedSimpleSampledCount =
        batchJobFailedNonUserErrorExpectedSimpleSampledCount;
    double finalIncrementalJobFailedUserErrorExpectedSimpleSampledCount =
        incrementalJobFailedUserErrorExpectedSimpleSampledCount;
    double finalIncrementalJobFailedNonUserErrorExpectedSimpleSampledCount =
        incrementalJobFailedNonUserErrorExpectedSimpleSampledCount;*/
    ...
    TestUtils.waitForNonDeterministicAssertion(30, TimeUnit.SECONDS, true, () -> {
      try {
        double batchJobSuccess = metrics.containsKey(".venice-cluster0--batch_push_job_success.Count")
            ...
            : 0.0;
        /*double batchJobSuccessSimpleSampledCount =
            metrics.containsKey(".venice-cluster0--batch_push_job_success.SimpleSampledCount")
                ? metrics.get(".venice-cluster0--batch_push_job_success.SimpleSampledCount").value()
                : 0.0;*/
        double incrementalJobSuccess = metrics.containsKey(".venice-cluster0--incremental_push_job_success.Count")
            ...
            : 0.0;
        /*double incrementalJobSuccessSimpleSampledCount =
            metrics.containsKey(".venice-cluster0--incremental_push_job_success.SimpleSampledCount")
                ? metrics.get(".venice-cluster0--incremental_push_job_success.SimpleSampledCount").value()
                : 0.0;*/
        double batchJobFailedUserError = metrics.containsKey(".venice-cluster0--batch_push_job_failed_user_error.Count")
            ...
            : 0.0;
        /*double batchJobFailedUserErrorSimpleSampledCount =
            metrics.containsKey(".venice-cluster0--batch_push_job_failed_user_error.SimpleSampledCount")
                ? metrics.get(".venice-cluster0--batch_push_job_failed_user_error.SimpleSampledCount").value()
                : 0.0;*/
        double batchJobFailedNonUserError =
            metrics.containsKey(".venice-cluster0--batch_push_job_failed_non_user_error.Count")
                ...
                : 0.0;
        /*double batchJobFailedNonUserErrorSimpleSampledCount =
            metrics.containsKey(".venice-cluster0--batch_push_job_failed_non_user_error.SimpleSampledCount")
                ? metrics.get(".venice-cluster0--batch_push_job_failed_non_user_error.SimpleSampledCount").value()
                : 0.0;*/
        double incrementalJobFailedUserError =
            metrics.containsKey(".venice-cluster0--incremental_push_job_failed_user_error.Count")
                ...
                : 0.0;
        /*double incrementalJobFailedUserSimpleSampledCount =
            metrics.containsKey(".venice-cluster0--incremental_push_job_failed_user_error.SimpleSampledCount")
                ? metrics.get(".venice-cluster0--incremental_push_job_failed_user_error.SimpleSampledCount").value()
                : 0.0;*/
        double incrementalJobFailedNonUserError =
            metrics.containsKey(".venice-cluster0--incremental_push_job_failed_non_user_error.Count")
                ...
                : 0.0;
        /*double incrementalJobFailedNonUserSimpleSampledCount =
            metrics.containsKey(".venice-cluster0--incremental_push_job_failed_non_user_error.SimpleSampledCount")
                ? metrics.get(".venice-cluster0--incremental_push_job_failed_non_user_error.SimpleSampledCount").value()
                : 0.0;*/
        assertEquals(
        ...
        /*assertEquals(
            incrementalJobSuccessSimpleSampledCount,
            finalIncrementalJobSuccessExpectedSimpleSampledCount,
            "Incremental push job success metric is incorrect");*/
        assertEquals(batchJobSuccess, batchJobSuccessExpected, "Batch push job success metric is incorrect");
        /*assertEquals(
            batchJobSuccessSimpleSampledCount,
            finalBatchJobSuccessExpectedSimpleSampledCount,
        ...
        /*assertEquals(
            batchJobFailedUserErrorSimpleSampledCount,
            finalBatchJobFailedUserErrorExpectedSimpleSampledCount,
            "Batch push job failed user error metric is incorrect");*/
        assertEquals(
        ...
        /*assertEquals(
            batchJobFailedNonUserErrorSimpleSampledCount,
            finalBatchJobFailedNonUserErrorExpectedSimpleSampledCount,
            "Batch push job failed non user error metric is incorrect");*/
        assertEquals(
        ...
        /*assertEquals(
            incrementalJobFailedUserSimpleSampledCount,
            finalIncrementalJobFailedUserErrorExpectedSimpleSampledCount,
            "Incremental push job failed user error metric is incorrect");*/
        assertEquals(
        ...
        /*assertEquals(
            incrementalJobFailedNonUserSimpleSampledCount,
            finalIncrementalJobFailedNonUserErrorExpectedSimpleSampledCount,
            "Incremental push job failed non user error metric is incorrect");*/
      } catch (Exception e) {
